...
# The adversaries learn to push agents away from the landmark
class Scenario(BaseScenario):
    ...
    def reset_world(self, world):
        ...
        for i, agent in enumerate(world.agents):
            # [adversary_0: landmark[goal_index[0]], adversary_1: landmark[goal_index[1]], agent_0: landmark[goal_index[0]], agent_1: landmark[goal_index[1]]]
            agent.goal_a = world.landmarks[goal_index[i % len(world.landmarks)]]
            agent.color = np.array([0.25, 0.25, 0.25])
            if agent.adversary:
                agent.color = np.array([0.75, 0.25, 0.25])
            else:
                j = agent.goal_a.index
                agent.color[j + 1] += 0.5
    ...
    def observation(self, agent, world):
        ...
        if not agent.adversary:
            ...
            return np.concatenate([agent.state.p_vel] + entity_pos + same_goal_good_agent_pos + other_pos)
