        ...
        if self.train_step % 2 == 0:
            ...
            with torch.no_grad():
                ...
                for agent_id in range(self.args.n_agents):
                    ...
                    obs_about_others_next = o_next[agent_id][:, -self.args.input_dim_others:]
                    if agent_id == self.agent_id:
            ...
            # the actor loss
            u[self.agent_id] = self.actor_network(o[self.agent_id][:, :self.args.input_dim_self], o[self.agent_id][:, -self.args.input_dim_others:])
