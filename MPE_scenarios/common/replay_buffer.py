...
class Buffer:
    ...
    def prior_sample(self, prior_buffer_size):
        temp_prior_buffer = {}
        index = np.random.randint(0, self.current_size, prior_buffer_size)
        for key in self.buffer.keys():
            if 'o' in key or 'u' in key:
                temp_prior_buffer[key] = self.buffer[key][index]
        # temp_prior_buffer.keys()=(o_1, o_2... u_1, u_2, ...)
        return temp_prior_buffer
