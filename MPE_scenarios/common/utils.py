...
def make_env(args):
    ...
    if 'simple_spread' in args.scenario_name:
        ...
        args.input_dim_self = 4 + 2 * args.n_landmarks
    elif 'defense' in args.scenario_name:
        # [p_vel + p_pos + relative pos to all landmarks + relative pos to corresponding good agents]
        args.input_dim_self = 4 + 2 * args.n_landmarks + 2 * 1
...
def handle_samples(samples, n_agents):
    ...
    u_cat = torch.from_numpy(np.array(u_cat)).float()
    return o_cat, u_cat
