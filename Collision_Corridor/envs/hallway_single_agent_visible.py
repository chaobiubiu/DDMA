...
class SingleHallwayVisible(tk.Tk, object):
    def __init__(self, args):
        super(SingleHallwayVisible, self).__init__()
        n_agents = 1
        rows, cols = 5, 7
        self.unit = 60
        self.obs_shape = 5 + 7
    ...
    def step(self, actions):
        ...
        if normal_actions[0] == 1:          # up
            if self.index[0][0] != 0:
                if not (self.index[0][0] == 2 and self.index[0][1] == 3):
                    self.index[0][0] -= 1
                    if obs1[1] > self.unit:
                        temp_pos1[1] -= self.unit
        elif normal_actions[0] == 2:        # down
            if self.index[0][0] != 4:
                if not (self.index[0][0] == 2 and self.index[0][1] == 3):
                    self.index[0][0] += 1
                    if obs1[1] < (self.rows - 1) * self.unit:
                        temp_pos1[1] += self.unit
        elif normal_actions[0] == 3:        # left
            if self.index[0][1] != 0:
                if not (self.index[0][1] == 4 and self.index[0][0] in [0, 1, 3, 4]):
                    self.index[0][1] -= 1
                    if obs1[0] > self.unit:
                        temp_pos1[0] -= self.unit
        elif normal_actions[0] == 4:        # right
            if self.index[0][1] != 6:
                if not (self.index[0][1] == 2 and self.index[0][0] in [0, 1, 3, 4]):
                    self.index[0][1] += 1
                    if obs1[0] < self.unit * (self.cols - 1):
                        temp_pos1[0] += self.unit
        self._update_obs()
        ...
        if self.index[0] == self.targets[0]:
            reward += 30
            done = True
        rewards = [reward]
        info = {}
