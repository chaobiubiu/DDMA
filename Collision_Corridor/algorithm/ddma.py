...
class DDMA:
    ...
    def load_pretrained_model(self, model_load_path):
        ...
        if os.path.exists(model_load_path + '/stage_1_0_actor_params.pkl'):
            ...
            if hasattr(self, 'expert_actor'):
                print('load single expert actor and critic')
    ...
    def pred_strength(self, inps):
        out, out_probs = self.detec(inps)
        strength = out_probs[:, 1]
        return strength
    ...
    def train(self, transitions, epsilon, other_agents):
        ...
        # the actor loss
        o_others = o[np.arange(self.args.n_agents) != self.agent_id, :, :]      # shape=(n_agents-1, batch_size, obs_shape)
        ...
        o_self_rep = o[self.agent_id].unsqueeze(dim=1).expand(-1, self.args.n_actions, -1).reshape(self.args.batch_size * self.args.n_actions, -1)
        # o_others.shape = (n_agents-1, batch_size, obs_shape)
        o_others_rep = o_others.unsqueeze(dim=2).expand(-1, -1, self.args.n_actions, -1).reshape(-1, (self.args.batch_size * self.args.n_actions), self.args.obs_shape)
        o_others_rep = o_others_rep.permute(1, 0, 2).reshape((self.args.batch_size * self.args.n_actions), -1)
        # u_onehot_others.shape = (n_agents-1, batch_size, n_actions)
        u_onehot_others = act_onehot[np.arange(self.args.n_agents) != self.agent_id, :, :]
        u_onehot_others_rep = u_onehot_others.unsqueeze(dim=2).expand(-1, -1, self.args.n_actions, -1).reshape(-1, (self.args.batch_size * self.args.n_actions), self.args.n_actions)
        u_onehot_others_rep = u_onehot_others_rep.permute(1, 0, 2).reshape((self.args.batch_size * self.args.n_actions), -1)
