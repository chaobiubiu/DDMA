...
import os
import numpy as np
from network.base_net import utility_network, vdn_network
from common.utils import hard_update, soft_update

# 将所有agent的utility network定义在该类中，vdn network仅有一个
class VDN:
    def __init__(self, args):
        ...
        self.mixing_network = vdn_network(args)
        self.target_mixing_network = vdn_network(args)
    ...
    def train(self, transitions, logger=None):
        ...
        q_total_next = self.target_mixing_network(target_q_utilities).detach()
        q_total = self.mixing_network(eval_q_utilities)
